...
function Product({ data, productStyle }) {
  ...
  const availableQuantity = checkAvaiableQuantityToAdd(cartState, data);
  ...
  const onAddToCart = (data) => {
    ...
    message.success("Product added to the cart successfully");
  };

  const onAddToWishlist = (data) => {
    if (productInWishlist) {
      dispatch(removeFromWishlist(data.id));
      return message.error("Product removed from the wishlist");
    } else {
      dispatch(addToWishlist(data));
      return message.success("Product added to the wishlist successfully");
    }
  };
