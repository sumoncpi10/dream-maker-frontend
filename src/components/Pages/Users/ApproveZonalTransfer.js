...
import { useZonalPostingApproveMutation } from '@/redux/features/user/userApi';
...
const ApproveZonalTransfer = ({ zonalRequestedUser }) => {
    ...
    const [zonalPostingApprove] = useZonalPostingApproveMutation();
    const handleDelete = (key) => {
        const newData = dataSource.filter((item) => item.mobileNo !== key);
        setDataSource(newData);
    };
    const handleRelease = async (mobileNo) => {
        const options = {
            id: mobileNo,
        }
        const result = await zonalPostingApprove(options);
        console.log(result)
